...
export default function SignupContent() {
  ...
  const handleSubmit = (e) => {
    ...
    } else if (emptySpace.test(userData.email)) {
      currentErrors.email = "*Email should not contain empty space.";
    }
    ...
    if (!userData.password) {
      ...
      emptySpace.test(userData.password)
    ) {
      switch (true) {
        ...
        case !specialChars.test(userData.password):
          ...
          case emptySpace.test(userData.password):
          currentErrors.password =
      ...
      currentErrors.email =`The email "${userData.email} is currently used."`;
      setAccountExist(true);
    }

    setError(currentErrors);

    if (Object.keys(currentErrors).length === 0 && !isEmailExist) {
  ...
  return (
    <>
      {accountExist && (
        ...
        >
          <p className="text-center"> The email is currently used </p>
        </AnimatedForm>
      )}
      <form
      ...
      >
        <InputContainer label="full name">
          ...
          {error.fullname && (
            <p className="text-[#d63a3a] text-[12px] mt-1">{error.fullname}</p>
          )}
        </InputContainer>
