...
const Input = ({
  ...
  const handleChange = (e) => {
    ...
    const matches = optionList.filter((option) =>
      option.name.toUpperCase().includes(newValue.toUpperCase()),
    );
  ...
  const handleSelectOption = (e) => {
    ...
    const valueCode = optionList.find(
      (option) => option.name.toUpperCase() === selectedValue.toUpperCase(),
    ).code;
            ...
            {filteredOptions.map((option) => {
              return (
                <li
                  ...
                  className="transition-all duration-300 py-1 px-[15px] capitalize w-fit cursor-pointer hover:translate-x-[5px] hover:opacity-70 text-(--text-primary)"
                >
                  {option.name}
                </li>
              );
