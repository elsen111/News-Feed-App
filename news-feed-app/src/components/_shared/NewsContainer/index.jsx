...
export default function NewsContainer({ newsPosts, nextPageId, filterParams = null, searchQuery = null, categoriesParam = null }) {
  ...
  const handleLoadMore = async() => {
    if (pathname === '/saved') {
      ...
      try {
        ...
        const morePosts = totalSavedPosts.slice(renderedPostsCount, renderedPostsCount + SAVED_PAGE_CHUNK);
  ...
  return (
    <section className="flex flex-col justify-center items-center gap-5 sm:gap-[35px]">
      { displayedNews.length == 0 && <p className="text-[16px]"> No news here </p>}
